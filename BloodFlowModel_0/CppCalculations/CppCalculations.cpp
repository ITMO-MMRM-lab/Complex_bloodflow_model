...
McCormackThread::McCormackThread(int _length, double* _dZ,  double _density, double _viscosity, double _friction_coeff):
	length(_length),density(_density),viscosity(_viscosity), friction_coeff(_friction_coeff)
{
	us_velocity_pred = new double[length];
	us_lumen_sq_pred = new double[length];
	us_pressure_pred = new double[length];
		 us_g_energy = new double[length];
...
void McCormackThread::calc(double* velocity, double* lumen_sq, double* pressure, double dt)
{
	...
	{	    
		...
		us_velocity_pred[i] = us_velocity_pred[i] - 1.0 / density / lumen_sq[i] * (friction_coeff * viscosity * PI * us_velocity_pred[i]) * dt;
	...
	for (int i = 1; i < L - 1; i++)
	{
	    ...
	    velocity[i] =  velocity[i] - 1.0 / 2.0 / density / lumen_sq[i] * (friction_coeff * viscosity * PI * velocity[i]) * dt;
